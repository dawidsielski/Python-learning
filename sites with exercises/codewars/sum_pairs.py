...
def sum_pairs(ints, s):
    for element in range(len(ints)):
        first_number = ints[element]
        second_element = s - first_number
        # print(ints[element + 1:])
        if second_element in ints[element + 1:]:
            print([first_number, second_element])
            return [first_number, second_element]
    return None
